...
A full-stack AI chat assistant — ask questions, upload files & images, and stream well-researched answers token-by-token. Chat history sidebar, dark mode, JWT auth, and a signature **cat-chases-mouse thinking animation** in the navbar (the cat catches the mouse the instant your answer finishes streaming).

> **Accuracy note:** answer quality comes from the underlying LLM provider (Anthropic Claude by default). The backend has a provider abstraction so you can swap providers via one env var.
...
| AI        | Anthropic Claude API (streaming) · OpenAI-compatible · mock  |
...
│   │   └── services/
│   │       ├── llm_service.py      # provider abstraction (anthropic/openai/mock) + streaming
...
│   │   │                           # Composer (+FileUpload), ThinkingIndicator, ThemeToggle
...
cp .env.example .env          # add your ANTHROPIC_API_KEY (or leave empty for mock mode)
...
2. Implement `stream_generate(messages)` as an async generator of text chunks
